...
Before subsampling, let's determine the smallest number of observations within any `roi` x `channel` group. This will inform our choice of subsampling size.
...
group_sizes <- df %>%
    group_by(roi, channel) %>%
    summarise(count = n(), .groups = "drop") # Use summarise with n()

# Find the minimum size
min_size <- min(group_sizes$count)

# Print the minimum size
print(paste("Minimum sample size across all roi x channel groups:", min_size))
...
Based on the minimum group size and the desire for balanced histograms, we will randomly sample a fixed number of observations from each `roi` x `channel` group. Let's choose a sample size (e.g., 1000, or `min_size` if it's smaller and still reasonably large).
...
df_subsampled <- df %>%
    group_by(roi, channel) %>%
    slice_sample(n = target_sample_size, replace = FALSE) %>% # Sample without replacement
...
# Verify counts per group in the subsampled data
# subsampled_counts <- df_subsampled %>% count(roi, channel)
# print(subsampled_counts)

```


## Histograms by color (Using Subsampled Data)
...
for (col in unique_colors_subsampled) {
    ...
    # Create the plot using the subsampled data
...
Now that we have subsampled data, we can perform statistical tests to compare the distribution of pixel values for each filament ROI against the corresponding Monarch ROI within each channel and color group. The Kolmogorov-Smirnov (K-S) test is suitable for comparing two empirical distributions.
...
```{r}
# Function to perform and format K-S test
run_ks_test <- function(monarch_data, filament_data, roi_filament, current_channel, current_color) {
...
# Get unique combinations of color and channel present in Monarch data
monarch_groups <- monarch_subsampled %>% distinct(color, channel)

# Initialize an empty list to store results
ks_results_list <- list()

# Loop through each Monarch color/channel group
for (i in 1:nrow(monarch_groups)) {
    current_color <- monarch_groups$color[i]
    current_channel <- monarch_groups$channel[i]

    # Get the Monarch data for this group
    monarch_group_data <- monarch_subsampled %>%
        filter(color == current_color, channel == current_channel)
    ...
    for (roi_filament in filament_rois_for_color) {
        # Get the filament data for this specific ROI and channel
        filament_group_data <- filament_subsampled %>%
            filter(roi == roi_filament, channel == current_channel)

        # Run the K-S test and store the result
        test_result <- run_ks_test(monarch_group_data, filament_group_data, roi_filament, current_channel, current_color)
...
ks_results_df <- bind_rows(ks_results_list) %>%
    arrange(color, channel, ks_statistic_D) # Sort by similarity (smaller D is better)
...
# Example: Find best match for each group
# best_matches <- ks_results_df %>%
#   group_by(color, channel) %>%
#   filter(!is.na(ks_statistic_D)) %>% # Ensure D is not NA
#   slice_min(order_by = ks_statistic_D, n = 1) %>%
#   ungroup()
# print("Best matches based on minimum K-S statistic (D):")
# print(best_matches)
